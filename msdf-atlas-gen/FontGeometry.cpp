...
FontGeometry::FontGeometry(const std::vector<GlyphGeometry> &glyphStorage)
    : geometryScale(1), metrics(),
      preferredIdentifierType(GlyphIdentifierType::UNICODE_CODEPOINT) {
  glyphs = glyphStorage;
...
FontGeometry::FontGeometry(FontGeometry &&orig) noexcept
    : geometryScale(orig.geometryScale), metrics(orig.metrics),
      preferredIdentifierType(orig.preferredIdentifierType),
      glyphs(orig.glyphs), rangeStart(orig.rangeStart), rangeEnd(orig.rangeEnd),
      glyphsByIndex(orig.glyphsByIndex),
      glyphsByCodepoint(orig.glyphsByCodepoint), kerning(orig.kerning),
      name(orig.name) {
  glyphs = orig.glyphs;
}

FontGeometry &FontGeometry::operator=(FontGeometry &&orig) noexcept {
  if (this != &orig) {
    ...
    glyphsByIndex = orig.glyphsByIndex;
    glyphsByCodepoint = orig.glyphsByCodepoint;
...
int FontGeometry::loadGlyphRange(msdfgen::FontHandle *font, double fontScale,
